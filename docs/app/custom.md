...
|:-------|:-------|:-------|:-------|:-------|
|[DR码](../framework/code.md)|DR:type,p1=v1,p2=v2,p3=v3,...|[DR:emoji, type=0, id=爱情]|返回DR码，支持图片、视频、文件、语音、音乐、卡片等，请参考[DR码](../framework/code.md)|普通版|
...
|热门文字|p1：文字类型，Joke（笑话）、Soup（鸡汤）、ColdKnow（冷知识）、DogDiary（舔狗日记）|[热门文字Joke]|返回笑话|VIP版|
|热门图片|p1：图片类型，meinv（美女）、shuaige（帅哥）、fengjing（风景）、dongman（动漫）|[热门图片meinv]|返回美女图片|VIP版|
|现行日期|无|[现行日期]|返回当前日期|VIP版|
|现行时间|无|[现行时间]|返回当前时间|VIP版|
|取随机数|p1：欲取随机数的最小值，p2：欲取随机数的最大值|[取随机数1,100]|返回1~100之间的随机数|VIP版|
|取运行目录|无|[取运行目录]|返回当前程序运行目录|VIP版|
|创建目录|p1：欲创建的目录名称|[创建目录D:\directory]|创建指定目录|VIP版|
|写配置项|p1：配置文件名，p2：节名称，p3：配置项名称，可空，p4：欲写入值，可空|[写配置项D:\config.ini,section,key,value]|写指定配置值到指定配置文件|VIP版|
|读配置项|p1：配置文件名，p2：节名称，p3：配置项名称，p4：默认文本，可空|[读配置项D:\config.ini,section,key]|读取指定配置文件配置值|VIP版|
|网页访问|p1：网址，p2：访问方式，可空，p3：提交信息，可空，p4：附加协议头，可空，p5：提交Cookies，可空|[网页访问https://open.drea.cc/openapi/res/get,1,FilePath=img/dongman/pc]|返回网页访问内容|VIP版|
|取文本之间|p1：被搜寻的文本，p2：开始文本，p3：结束文本|[取文本之间我爱你,我,你]|返回取文本之间的文本|VIP版|
